...
本次更新对核心的**智能凭证管理系统**进行了重大重构和增强，显著提升了系统的稳定性、公平性和智能化水平。

- **重构凭证轮换逻辑**：彻底修复了凭证轮换不公的底层缺陷。旧机制在高并发和动态凭证加载时会导致索引重置，使部分凭证被过度使用，而新凭证得不到调用。新架构采用“稳定主列表 + 动态运行时跳过”模式，确保所有可用凭证都能被公平、均匀地轮换调用。

- **实现API驱动的动态禁用**：引入了基于 Google API 返回的 `quotaResetTimeStamp` 的毫秒级精确禁用机制。当凭证因 `429` 错误被临时禁用时，系统能精确计算其解禁时间，并自动恢复，取代了过去笼统的固定时长禁用策略。

- **修复并发竞态条件**：在高并发场景下，多个请求同时收到 `429` 错误时，旧的“读-改-写”状态文件操作会引发竞态条件，导致只有一个凭证被成功禁用。通过引入 `asyncio.Lock`，确保了状态更新的原子性，保证所有出错的凭证都能被正确、即时地处理。

- **增强日志与可调试性**：
  - 将所有日志时间戳转换为更易读的 **北京时间 (UTC+8)** 格式。
  - 在关键错误日志（如 `429` 错误）中增加了**凭证索引**，极大地方便了快速定位和调试特定凭证的问题。

- **修复“僵尸数据”问题**: 彻底解决了凭证在删除后，其空记录依然残留导致前端显示异常的严重bug。通过在 `CredentialManager` 中引入智能清理机制，系统现在可以自动检测并彻底清除这些无效的“僵尸”数据，保证了数据存储的一致性和系统的健壮性。

这些改进使凭证管理系统在高负载和不稳定网络环境下更加健壮和可靠。
